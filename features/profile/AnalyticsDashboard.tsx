...
export const AnalyticsDashboard: React.FC = () => {
  const { activityLog, completedSteps, engagementScore } = useAppStore();

  const eventCounts = activityLog.reduce((acc, curr) => {
    const type = curr.type.split('_')[0];
  ...
  const stats = [
    ...
    { label: "Interactions", value: activityLog.length, icon: Activity, color: "text-emerald-500" },
    ...
    <div className="space-y-6">
      ...
      <Card className="p-6 bg-white border-none shadow-lg shadow-slate-200/40">
        <div className="flex items-center justify-between mb-6">
          ...
          <span className="text-[9px] font-black text-slate-400 uppercase">Last 20 events</span>
        </div>
              ...
              <div key={i} className="flex items-center justify-between p-3 bg-slate-50 rounded-xl border border-slate-100">
                <div className="flex items-center gap-3">
                  <div className={cn(
                    ...
                    log.type.includes('assistant') ? "bg-blue-500" :
                      log.type.includes('map') ? "bg-emerald-500" :
                        log.type.includes('step') ? "bg-amber-500" : "bg-slate-500"
                  )} />
